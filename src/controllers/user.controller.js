...
const loginUser = asyncHandler(async(req,res) => {
    ...
    //Avi rite pan kari sakay return line break karine
    return res
    .status(200)
...
const getUserChanelProfile = asyncHandler(async(req,res) => {
    ...
    const chanel = await User.aggregate([
        ...
        },
        {
            $addFields: {
                ...
                isSubscribed: {
                    $cond:{
                        if: {$in:[req.user?._id,"subscribers "]},  //in mean present or not
                        then: true,
                        else: false
                    }
                }
        ...
        {
            //$project means selected filled apse badhi fill nahi ape
            $project:{
                fullName: 1,
                username: 1,
                subscribersCount: 1,
    ...
    if(!chanel?.length){
        throw new ApiError(404,"Chandel not exists")
    }

    return res
    .status(200)
    .json(new ApiResponse(200,"user chanel fetched successfully"))
})
        ...
        },
        {
            $lookup: {
                ...
                pipeline:[
                    ...
                    //second pipeline threw owner field ak j kadhi badhi nahi
                    {
                        $addFields: {
                            owner: {
                                $first: "$owner"
